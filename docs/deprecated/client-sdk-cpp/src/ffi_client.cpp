...
proto::FfiResponse FfiClient::SendRequest(const proto::FfiRequest &request) const {
   ...
   if (handle == INVALID_HANDLE) {
        std::cout << "ERROR: Received INVALID_HANDLE from FFI!" << std::endl;
...
void FfiClient::PushEvent(const proto::FfiEvent &event) const {
    ...
    for (auto& [_, listener] : listeners_) {
        try {
            ...
            std::cerr << "Unhandled exception in FFI listener: " << e.what() << std::endl;
        } catch (...) {
            std::cerr << "Unhandled unknown exception in FFI listener" << std::endl;
        }
    }
}
...
void LivekitFfiCallback(const uint8_t *buf, size_t len) {
    try {
        ...
        std::cerr << "Exception while handling FFI callback: " << e.what() << std::endl;
    } catch (...) {
        std::cerr << "Unknown exception while handling FFI callback" << std::endl;
    }
}
