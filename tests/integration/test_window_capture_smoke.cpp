...
TEST(WindowCaptureIntegrationTest, CaptureOneWindowFrame)
{
    ...
    if (!links::core::isWindowShareSupportedOnCurrentPlatform()) {
        GTEST_SKIP() << "Window share is unsupported on current platform/session.";
    }

    auto capturer = links::desktop_capture::DesktopCapturer::createWindowCapturer();
    if (!capturer) {
        GTEST_SKIP() << "Window capturer is unavailable.";
    }

    links::desktop_capture::DesktopCapturer::SourceList sources;
    if (!capturer->getSourceList(&sources) || sources.empty()) {
        GTEST_SKIP() << "No capturable window source available.";
    }
